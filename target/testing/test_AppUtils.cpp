...
BOOST_AUTO_TEST_CASE( SplitUri )
{
  ...
  url = "interspec://decay/chain?nuclide=U238&activity=3uCi&initialage=20y&timespan=22y&actunits=ci";
  BOOST_REQUIRE_NO_THROW( AppUtils::split_uri( url, host, path, query_str, fragment ) );
  ...
  BOOST_REQUIRE_NO_THROW( AppUtils::split_uri( url, host, path, query_str, fragment ) );
  BOOST_CHECK( host == "G0" );
  BOOST_CHECK( path == "000/NCF60UV/PGV2ER2*TFY1GQIN%ZQLOQMTVN4S0ZV6QDIQDJ$D..." );
...
/*
  url = "";
 BOOST_REQUIRE_NO_THROW( AppUtils::split_uri( url, host, path, query_str ) );
  BOOST_CHECK( host == "" );
  BOOST_CHECK( path == "" );
  BOOST_CHECK( query_str == "" );
