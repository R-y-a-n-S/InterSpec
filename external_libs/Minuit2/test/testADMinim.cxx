                     ...
                     double &minval, double &edm, std::vector<double> & xmin, std::vector<double> &covMat)
{
   ...
   int ndim = func.GetNpar(); // dim is in the parameters


   auto fcn = [&](const double * x){
      return func.EvalPar(nullptr,x);
   };
   auto gradFcn = [&](const double *x, double *g) {
...
int testADMinimizer(std::string formula, const double *x0, const double *s0)
{

   std::cout << "\n************************************************************\n";
   std::cout << "\tTest using formula " << formula << std::endl << std::endl;
...
int testRosenBrock()
{
   ...
   iret |= testADMinimizer(formula, xRB, s0);
